...
1. **Always confirm before deleting**: Show user what will be deleted
2. **Use UUIDs for precision**: When deleting specific readings (iOS)
3. **Check permissions**: Ensure WRITE access before deletion
4. **Handle errors gracefully**: Deletion may fail silently
5. **Consider undo**: Store deleted data temporarily for undo functionality
6. **Be careful with ranges**: Time-range deletion affects all data in that period
