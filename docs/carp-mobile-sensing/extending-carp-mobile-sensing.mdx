...
If you want to add new sensing capabilities you would basically create a new [`SamplingPackage`](https://pub.dev/documentation/carp_mobile_sensing/latest/domain/SamplingPackage-class.html). A sampling package implements the following classes:
...
In the following, we will use the [`DeviceSamplingPackage`](https://pub.dev/documentation/carp_mobile_sensing/latest/sampling_packages/DeviceSamplingPackage-class.html) as an example of a sampling package that does not use any external device.

### Sampling package

The first step is that your new sampling package should implement the [`SamplingPackage`](https://pub.dev/documentation/carp_mobile_sensing/latest/domain/SamplingPackage-class.html) interface:
...
<Note>
  ...
  In the measures used as an example above, no permissions are required. However, in the [`SensorSamplingPackage`](https://pub.dev/documentation/carp_mobile_sensing/latest/sampling_packages/SensorSamplingPackage-class.html), the `activityRecognition` permission is required in order to collect step counts, as shown in the [`samplingSchemes`](https://pub.dev/documentation/carp_mobile_sensing/latest/sampling_packages/SensorSamplingPackage/samplingSchemes.html) of that package.
...
Sampling configurations can be part of a study protocol where you can [override](https://pub.dev/documentation/carp_core/latest/common/Measure/overrideSamplingConfiguration.html) the sampling configuration of a measure.

### Data

Next, you should define the package-specific [`Data`](https://pub.dev/documentation/carp_core/latest/common/Data-class.html) that the package collects.
...
The next step is to implement probes for each measure type. A probe collects the data and returns it as [`Measurement`](https://pub.dev/documentation/carp_core/latest/data/Measurement-class.html).
...
Implement the [`DataManager`](https://pub.dev/documentation/carp_mobile_sensing/latest/domain/DataManager-class.html) interface which uploads or saves the measurement as they are sampled.
</Step>
<Step title="Create a data manager factory">
Create a [`DataManagerFactory`](https://pub.dev/documentation/carp_mobile_sensing/latest/domain/DataManagerFactory-class.html) that can create a data manager based on the data endpoint configuration,
</Step>
</Steps>
...
Any new data manager should implement the [`DataManager`](https://pub.dev/documentation/carp_mobile_sensing/latest/domain/DataManager-class.html) interface:
...
All of these methods have to be implemented. However, the [`AbstractDataManager`](https://pub.dev/documentation/carp_mobile_sensing/latest/domain/AbstractDataManager-class.html) class provides a useful class to start from.
...
For a data manager to be used in CAMS, its factory must be registered in the [`DataManagerRegistry`](https://pub.dev/documentation/carp_mobile_sensing/latest/domain/DataManagerRegistry-class.html) singleton like this:
...
For example the following `OMHGeopositionDataPoint` class represents an OMH [`Geoposition`](https://pub.dev/documentation/openmhealth_schemas/latest/openmhealth_schemas/Geoposition-class.html) data point.
