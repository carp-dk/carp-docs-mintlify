...
In comparison to an [`BackgroundTask`](https://pub.dev/documentation/carp_core/latest/common/BackgroundTask-class.html), an [`AppTask`](https://pub.dev/documentation/carp_mobile_sensing/latest/domain/AppTask-class.html) can be configured with user-facing properties:
...
In the PulmonaryMonitor, access to enqueued tasks is handled in [`sensing_bloc.dart`](https://github.com/carp-dk/pulmonary_monitor_app/blob/master/lib/bloc/sensing_bloc.dart).
...
This is typically done during the initialization of CAMS in the app. In PulmonaryMonitor, it happens when the `Sensing` class is created in [sensing.dart](https://github.com/carp-dk/pulmonary_monitor_app/blob/c2013a17a6e567be05490719e05d263cea18112b/lib/sensing/sensing.dart#L55).
